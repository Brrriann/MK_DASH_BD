...
export async function POST(
  ...
  const client = (estimate as Estimate & { clients: { contact_name: string; email: string; company_name: string } | null }).clients;
  if (!client?.email) {
    return NextResponse.json({ error: "클라이언트 이메일이 없습니다. 고객 정보에서 이메일을 먼저 등록해주세요." }, { status: 422 });
  }
  ...
  // Update status to "pending" (=발송됨) if currently expired or unset
  await supabase
    .from("estimates")
